...
'###S3.3  input num_var
  num_var = crt.Dialog.Prompt("1/2:输入重复执行次数(0-2147483647 )", _
                                  "重复次数", "50", False)
 If num_var = "" Then 
...
'###S3.4  set time_var
'  time_var = crt.Dialog.Prompt("2/2:输入重复执行间隔时间(ms)", _
'                                  "间隔时间", "1000", False)
'  If time_var = "" Then 
'  	MsgBox "系统退出，Byebye! ",,"系统提示"
'  	exit Sub
...
Do Until loop_num > CLng(num_var)  
    ...
    For nIndex = 1 to crt.GetTabCount
        ...
        if objCurrentTab.Session.Connected = True then
            ...
            'Step7: do sth
	          'count
	          objCurrentTab.Screen.Send "### add: " & loop_num & chr(13)
	          objCurrentTab.Screen.WaitForString  "]"
...
Do Until loop_num > CLng(num_var)  
    ...
    For nIndex = 1 to crt.GetTabCount
        ...
        if objCurrentTab.Session.Connected = True then
            ...
            'Step7: do sth
	          'count
	          objCurrentTab.Screen.Send "### del: " & loop_num & chr(13)
	          objCurrentTab.Screen.WaitForString  "]"
