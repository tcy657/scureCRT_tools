...
Set objCurrentTab = objCurrentTab.GetTab(1)
...
Function del_vpws_b1
	...
	objCurrentTab.Screen.WaitForString "(config-if)#"
	objCurrentTab.Screen.Send "shutdown" & chr(9) & chr(9) & chr(13)
	objCurrentTab.Screen.WaitForString "(config-if)#"
	objCurrentTab.Screen.Send "exit" & chr(13)
	objCurrentTab.Screen.WaitForString "(config)#"
...
Set g_objTab = objCurrentTab.GetScriptTab
...
Sub Main()
    ...
    Dim nIndex
    For nIndex = start to objCurrentTab.GetTabCount
        Set objCurrentTab = objCurrentTab.GetTab(nIndex)
