...
'
' Description:
'   This script example is designed to run while connected device.
'    IP generate:
'   Key1 key2 = 10.key1.key2.1
'      
...
Sub Main()
    ...
    crt.Screen.Synchronous = True
	    ...
	    'crt.Screen.Send "agent dump-my-idip" & chr(13)
	    'crt.Screen.WaitForString "fiberhome#"
	
'My SN Information: 
'------------------------
'SN: 012345678912
'Device Type:0 ID1:8 ID2:9 Area Flag:0x80
   	
   	'第一步，获取SN号
   	szData = CaptureOutputOfCommand("agent idip 0", "fiberhome#")

    '获取法1，SN: 012345678912
    'szData = Replace(szData, "My SN Information:", "")   
    'szData = Replace(szData, "-", "")   
    'szData = Replace(szData, "SN: ", "") 
    ''szData = Replace(szData, "Device Type:", "")   '二进制比较从字符串左端开始。返回 "XXYXXPXXY"。
    'szData = Left(szData, 18) '
    
    '获取法2，SN: 012345678912
    szData = mid(szData,53,12)
    'MsgBox  szData

 '第二步，输入两个Key值
  Key = crt.Dialog.Prompt("输入两个Key值", _
                                  "Key1, Key2", "x y", False)
  If Key = "" Then 
  	MsgBox "Key为空，系统退出，Byebye! ",,"系统提示"
  	exit Sub
  End If
  
     '第三步，生成NMU IP
     params = Split( Key," ", -1, 1 )
		 '取两个Key值，组成NMU IP = 10.key1.key2.1
		 Key_1 = params(0)
		 Key_2 = params(1)
		 IP_1 = "10." & Key_1 & "." & Key_2 & ".1"
     
    '第四步，设置NMU IP
    crt.Screen.Send  "agent idip-set sn " & szData & " key1 " & Key_1 & " key2 " & Key_2 & " default-area-flag 1 area-type 1 ip-address " & _
       IP_1 & " mask 255.255.255.0 priority 1"  & chr(13)
    crt.Screen.WaitForString "fiberhome#"
