...
'***************************************
' name: 上传-针对xex延时-2th.vbs
' time: 2014-08-02 17:07:05
' others: writen by cytao for xiyang
' 
'**************************************
...
REM	crt.Screen.Send "telnet 10.26.0.5" & chr(13)
	...
	REM crt.Screen.WaitForString "->"
	 ...
	 '获取文件后缀，以符号"."为分裂符
   comp_xex = Split(Contents(i),".", -1, 1 )
	 '比较
	 comp =comp_xex(UBound(comp_xex))
	 result =  StrComp("xex",comp)
   'MsgBox  result
     If result = 0 Then	
        crt.Screen.Send "taskDelay 5000 " & chr(13)
     End If
        crt.Screen.Send chr(13)
	Next
